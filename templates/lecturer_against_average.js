(function(){
    ...
    var average_data = d3.csvParse(d3.select('#data_average').html());

    var module_data = d3.csvParse(d3.select('#data_agreement_overall').html());
    ...
    var questions = comparison_data.map(function(d) { return d.question; });

    yScale.domain(questions);
    xScale.domain([d3.min(comparison_data, function(d){
        return d.module < d.average ? d.module : d.average;
    }), 1.05]);
    colourScale.domain([d3.min(comparison_data, function(d){
        return d.diff;
    }), 0, d3.max(comparison_data, function(d){
        return d.diff;
    })]);
